...
DOCKER_DB_CONTAINER := dbezaccmmmx
MYSQL := mysql
...
serveez: stopez ## Clear the cache and start the web server
	...
	@cd $(EZ_DIR) && $(SYMFONY) run -d --watch=ezplatform/config,ezplatform/src,ezplatform/vendor,bundle symfony console messenger:consume ezaccelerator
