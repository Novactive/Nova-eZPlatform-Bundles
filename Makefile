...
installez: ## Install eZ as the local project
	@docker run -d -p 3339:3306 --name ezdbnovaezprotectedcontentcontainer -e MYSQL_ROOT_PASSWORD=ezplatform mariadb:10.2
...
serveez: stopez ## Clear the cache and start the web server
	...
	@docker start ezdbnovaezprotectedcontentcontainer
	@cd $(EZ_DIR) && bin/console cache:clear
...
stopez: ## Stop the web server if it is running
	...
	@docker stop ezdbnovaezprotectedcontentcontainer
